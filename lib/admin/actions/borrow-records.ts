...
export const updateBorrowRecordStatus = async ({id, status}: {id: string; status: "BORROWED" | "RETURNED";}) => {
    try {
        ...
        return {success: true, data: updatedBorrowRecord[0] as BorrowRecordData};
    } catch (error) {
        console.log(error);
        return {success: false, error: "Failed to update borrow record status"};
    }
};
...
export const getBorrowRequestStats = async () => {
    ...
    return {totalBorrowRequest: totalBorrowRequest[0].count, teloranceBorrowRequest};
};
