...
import {CirclePlus, Eye, PenLine, SquareArrowOutUpRight, Trash2} from "lucide-react";
import {Dialog, DialogContent, DialogTitle, DialogTrigger} from "../ui/dialog";
import {IKImage} from "imagekitio-next";
import config from "@/lib/config";
import {useState} from "react";
import GridLoader from "react-spinners/GridLoader";
import {ApproveUser, changeUserRole, deleteUser, getAccountRequestData, getAllUsers, rejectUser} from "@/lib/admin/actions/user";
import ActionDialog from "./ActionDialog";
import {toast} from "@/hooks/use-toast";
import {ScaleLoader} from "react-spinners";
import BookCover from "./BookCover";
import {deleteBook, getAllBooks} from "@/lib/admin/actions/book";
import {updateBorrowRecordStatus} from "@/lib/admin/actions/borrow-records";
...
export const AllUsersColumns: ColumnDef<User>[] = [
    {
        ...
        header: "Name",
        cell: ({row}) => (
            <UserAvatar name={row.original["fullName"]} email={row.getValue("email")}/>
        ),
    },
    {
    ...
    },
    {
        ...
        cell: ({row, table}) => {
            ...
            const handleRoleChange = async (newRole: "ADMIN" | "USER") => {
                setLoading(true);
                try {
                    const result = await changeUserRole({userId: row.original.id, role: newRole});
                    ...
                    toast({title: "Error", description: error instanceof Error ? error.message : "Failed to change role", variant: "destructive"});
                } finally {
                    setLoading(false);
                }
            };
            return (
                <DropdownMenu>
                    <DropdownMenuTrigger asChild disabled={loading}>
                        <div className={`px-3 py-2 rounded-full w-min cursor-pointer flex items-center justify-center ${row.getValue("role") === "USER" ? "bg-red-200" : "bg-green-200"}`}>
                            {loading ? (
                        ...
                        {/* <DropDownOption color={DropDownOptions.user_role[row.getValue('role')]} name={row.getValue('role')}/> */}
                    </DropdownMenuTrigger>
                    <DropdownMenuContent>
    ...
    },
    {
        ...
        cell: ({row, table}) => {
            ...
            const handleAction = async (e: React.MouseEvent<HTMLButtonElement>): Promise<void> => {
                ...
                setLoading(true);
                try {
                    const result = isApprove ? await ApproveUser({id: row.original.id}) : await rejectUser({id: row.original.id});
                    ...
                    toast({
                        title: "Success",
                        description: isApprove ? "User approved successfully" : "User rejected successfully",
                    });
                    ...
                    toast({
                        title: "Error",
                        description: error instanceof Error ? error.message : `Failed to ${target.value} user`,
                        variant: "destructive",
                    });
                } finally {
                    setLoading(false);
...
export const BooksColumns: ColumnDef<BookListData>[] = [
    {
        ...
        cell: ({row}) => (
            <div className="flex items-center gap-2">
                <BookCover coverColor={row.original.coverColor} coverImage={row.original.coverUrl} varient="extraSmall"/>
...
export const BorrowRecordsColumns: ColumnDef<BorrowRecordData>[] = [
    {
        ...
        cell: ({row}) => (
            <div className="flex items-center gap-2">
                <BookCover coverColor={row.original.bookCoverColor || ""} coverImage={row.original.bookCoverUrl || ""} varient="extraSmall"/>
    ...
    },
    {
        ...
        cell: ({row}) => (
            <UserAvatar name={row.original.userFullName || "Unknown User"} email={row.original.userEmail || ""}/>
        ),
    },
    {
        accessorKey: "status",
